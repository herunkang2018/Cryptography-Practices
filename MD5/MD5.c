...
{
	unsigned long int a = state[0], b = state[1], c = state[2], d = state[3], x[16];//子分组 
...
//更新state，作为下一次子分组的常量（幻数），最后一组的幻数就是摘要 
	state[0] += a;
	state[1] += b;
	state[2] += c;
	state[3] += d;
...
void MD5_Update( MD5_State *s, unsigned char *input, unsigned int inputLen )
{
	...
	//! 字节数 mod 64 // index 会变化 分别是原始长度 padding length_info 
	//c[0]: 0->8  ->448  ->512
	//index:0->1  ->56   //->64
	//part:64->63 ->8 
	index = (unsigned int)((s->count[0] >> 3) & 0x3F);//>> << 3
	printf("INDEX is:%d\n", index);

	//六十四位系统下，直接用uint64即可，但是为了兼容性
	//下述方法 精度有问题 
	//! 更新位数
	//低位在前 表示的是位数 
	if((s->count[0] += ((unsigned long int)inputLen << 3))//s->count[0] is already added 
		< ((unsigned long int)inputLen << 3)){// 模拟进位 因为是无符号的进位 
			s->count[1]++;
		}
			printf("count 1 1 is:%d\n", s->count[0]);
	s->count[1] += ((unsigned long int)inputLen >> 29); 
	printf("count 1 2 is:%d\n", s->count[1]);
	partLen = 64 - index;// index 刚开始是 0  
	printf("PARTLEN is %d\n", partLen);
	...
	//! MD5 4轮运算
	//一直进行下去 直到可能留有尾巴 
	if (inputLen >= partLen)// 8 == 8// 此处，inputlen累加到一定程度，也就是可以大于64，那么进行trans 
	{
		//实际的input放到输入缓冲区中 
		MD5_memcpy( (unsigned char*)&s->buffer[index],  (unsigned char*)input, partLen ); 
		MD5_Transform( s->state, s->buffer );// block -> states trans一轮 

		for( i = partLen; i + 63 < inputLen; i += 64 )//trans N轮 
			MD5_Transform( s->state, &input[i] );// 按照partLen 会一直计算到需要填充或者追加长度 
		printf("i is %d\n", i); 
		index = 0;
	}
	else
...
{
	unsigned char bits[8];//计算真实的长度 
	unsigned int index, padLen;

	Encode (bits, s->count, 8);//4 bytes and other 4 bytes

	//! 长度小于448位(mod 512),对明文进行填充
	//PADDING HERE mod 64
	index = (unsigned int)((s->count[0] >> 3) & 0x3f);
	padLen = (index < 56) ? (56 - index) : (120 - index);//需要填充的字节数 
	printf("PADDING %d \n", padLen);//
	MD5_Update( s, s->PADDING, padLen );//

	MD5_Update( s, bits, 8);//需要把长度放在尾部 
